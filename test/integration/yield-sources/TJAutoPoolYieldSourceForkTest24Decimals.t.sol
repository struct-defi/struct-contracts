...
contract AutoPoolYieldSource24Decimals_IntegrationTest is YieldSourceBaseTestSetup {
    IERC20Metadata tokenA;
    IERC20Metadata tokenB;
    address strategyOwner = 0x6580CaE5dd7C2fE67132D2D7DcaADB3896fEAA9d;

    function setUp() public virtual override {
        vm.createSelectFork(vm.envString("MAINNET_RPC"), 34918334);
        autoPoolVault = autoPoolVault_WETH_AVAX;
        super.setUp();
        tokenA = sut.tokenA();
        tokenB = sut.tokenB();
    }
    ...
    function testSupplyTokensAP_ShouldEmitTokensSuppliedEvent() public {
        ...
        vm.expectEmit(false, false, false, false, address(sut));
    ...
    function testQueueForRedemptionAP_ShouldUpdateTotalAPTokenShares() public {
        ...
        IAPTFarm.UserInfo memory _userInfo = aptFarm.userInfo(sut.aptFarmId(), address(sut));
    ...
    function testRecompoundRewardsAP_WithSingleReward() public {
        ...
        _userInfo = aptFarm.userInfo(sut.aptFarmId(), address(sut));
    ...
    function testRecompoundRewardsAP_ShouldSwapRewards1_WithSingleReward() public {
        ...
        uint256 _harvestedTokenBValue = _getPrice(address(tokenB)) * (_harvestedTokenB) / 1e18;
    ...
    function testRecompoundRewardsAP_ShouldNotSwapRewards_WithDualReward_NonNative() public {
        ...
        aptFarm.set(2, 12400793000000000, IRewarder(address(_mockRewarder)), true);
    ...
    function testRecompoundRewardsAP_ShouldRevert_IfInvalidRate() public {
        ...
        vm.mockCall(
            ...
            abi.encodeWithSelector(IStructPriceOracle.getAssetPrice.selector, address(wavax)),
            abi.encode(5e18)
        );
        _simulateSupply(address(tokenA), address(tokenB), sut, mockProduct);
        vm.warp(block.timestamp + 0.5 days);
    ...
    function testRecompoundRewardsAP_WithoutFarm() public {
        ...
        _simulateSupply(
            address(autoPoolVaultWithoutFarm.getTokenX()),
            address(autoPoolVaultWithoutFarm.getTokenY()),
            sut2,
            mockProduct
        );
        vm.warp(block.timestamp + 0.5 days);
    ...
    function testUpdateFarmInfoAP_ShouldUpdateFarmInfo_DualReward_NonNative() public {
        ...
        vm.startPrank(0x2fbB61a10B96254900C03F1644E9e1d2f5E76DD2); // APTFarm Owner
        aptFarm.set(2, 12400793000000000, IRewarder(address(_mockRewarder)), true);
        vm.stopPrank();
