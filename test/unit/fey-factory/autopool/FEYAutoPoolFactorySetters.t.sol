...
import "../../../common/fey-factory/autopool/FEYFactoryBaseTestSetup.sol";

contract FEYAutoPoolFactoryFSetters_UnitTest is FEYFactoryBaseTestSetup {
    ...
    function testSetter_setYieldSource_Success() public {
        ...
        admin.setYieldSource(address(autoPoolVault), address(yieldSource));
        address _newYieldSource = address(sut.yieldSources(address(autoPoolVault)));
    ...
    function testSetter_setYieldSource_RevertInvalidAccess() public {
        ...
        console.log("should revert if called by non-governance role account");
        vm.expectRevert(abi.encodePacked(Errors.ACE_INVALID_ACCESS));
        user1.setYieldSource(address(autoPoolVault), address(yieldSource));
    }
    ...
    function testSetter_setYieldSource_RevertIfInvalidAddress() public {
        ...
        admin.setYieldSource(address(autoPoolVault), address(0));
    }

    function testSetter_setYieldSource_RevertIfTokensDoesntMatch() public {
        console.log("should revert if underlying tokens of yield source and autopool vault doesn't match");

        address BTC_AVAX_VAULT = 0x1C739A43606794849750C50bC7C43FBbDAcdf801;

        vm.mockCall(
            address(BTC_AVAX_VAULT),
            abi.encodeWithSelector(IAutoPoolVault.getTokenX.selector),
            abi.encode(address(0x152b9d0FdC40C096757F570A51E494bd4b943E50))
        );

        vm.mockCall(
            address(BTC_AVAX_VAULT),
            abi.encodeWithSelector(IAutoPoolVault.getTokenY.selector),
            abi.encode(address(wavax))
        );
        vm.expectRevert(abi.encodePacked(Errors.AE_INVALID_YIELDSOURCE));
        admin.setYieldSource(address(BTC_AVAX_VAULT), address(yieldSource));
    }
    ...
    function testSetter_setPoolStatus_Success() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit PoolStatusUpdated(address(autoPoolVault), 1, address(wavax), address(usdc));
        admin.setPoolStatus(address(autoPoolVault), 1);
    }
    ...
    function testSetter_setPoolStatus_RevertInvalidAccess() public {
        ...
        vm.expectRevert(abi.encodePacked(Errors.ACE_INVALID_ACCESS));
        user1.setPoolStatus(address(autoPoolVault), 1);
    }
