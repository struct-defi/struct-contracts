...
contract FEYAutoPoolFactoryCreateProduct_UnitTest is FEYFactoryBaseTestSetup {
    ...
    function onSetup() public virtual override {
        ...
        vm.mockCall(
            address(autoPoolVault), abi.encodeWithSelector(IAutoPoolVault.isDepositsPaused.selector), abi.encode(false)
        );
    }
    ...
    function testCreateProduct_RevertAutoPoolPaused() public {
        console.log("should if the underlying autopool vault deposits are paused");
        setupFactoryState();
        vm.mockCall(
            address(autoPoolVault), abi.encodeWithSelector(IAutoPoolVault.isDepositsPaused.selector), abi.encode(true)
        );
        vm.expectRevert(abi.encodePacked(Errors.VE_AUTOPOOLVAULT_PAUSED));
        user1.createProductAndDeposit(address(wavax), address(usdc), SENIOR_TRANCHE, 0);
    ...
    function testCreateProduct_RevertInactivePool() public {
        ...
        deal(address(user1), initialDepositAmount);
        setupFactoryState();
        sut.setPoolStatus(address(autoPoolVault), 2);
        vm.expectRevert(abi.encodePacked(Errors.VE_INVALID_POOL));
        user1.createProductAndDeposit(address(wavax), address(usdc), SENIOR_TRANCHE, initialDepositAmount);
    }

    function testCreateProduct_RevertYieldSourceNotSet() public {
        console.log("product creation should revert if YieldSource contract is not set for the pool");
        deal(address(user1), initialDepositAmount);
        // set factory state except yield source contract
        spToken.setFeyProductFactory(sut);
        sut.setTokenStatus(address(wavax), 1);
        sut.setTokenStatus(address(usdc), 1);
        sut.setPoolStatus(address(autoPoolVault), 1);
        vm.expectRevert(abi.encodePacked(Errors.AE_YIELDSOURCE_NOT_SET));
        user1.createProductAndDeposit(address(wavax), address(usdc), SENIOR_TRANCHE, initialDepositAmount);
    }
}
